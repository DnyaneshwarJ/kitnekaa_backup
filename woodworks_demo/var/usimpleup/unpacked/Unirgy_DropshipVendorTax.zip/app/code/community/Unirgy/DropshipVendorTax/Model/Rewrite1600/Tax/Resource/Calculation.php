 ...
 */
class Unirgy_DropshipVendorTax_Model_Rewrite1600_Tax_Resource_Calculation extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
     * Rates cache
     *
    ...
    protected function _getRates($request)
    {
        ...
        // Process productClassId as it can be array or usual value. Form best key for cache.
        $vendorClassId = $request->getVendorClassId();
        $ids = is_array($vendorClassId) ? $vendorClassId : array($vendorClassId);
        foreach ($ids as $key => $val) {
            $ids[$key] = (int) $val; // Make it integer for equal cache keys even in case of null/false/0 values
        }
        $ids = array_unique($ids);
        sort($ids);
        $vendorClassKey = implode(',', $ids);

        // Form cache key and either get data from cache or from DB
        $cacheKey = implode('|', array($storeId, $customerClassId, $productClassKey, $vendorClassKey, $countryId, $regionId, $postcode));
        ...
        if (!isset($this->_ratesCache[$cacheKey])) {
            ...
            if ($vendorClassId) {
                $select->where('vendor_tax_class_id IN (?)', $vendorClassId);
            }
