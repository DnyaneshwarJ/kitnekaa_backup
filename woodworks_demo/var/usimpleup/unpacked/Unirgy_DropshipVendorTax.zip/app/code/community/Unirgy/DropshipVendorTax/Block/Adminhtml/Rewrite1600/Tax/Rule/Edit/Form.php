...
class Unirgy_DropshipVendorTax_Block_Adminhtml_Rewrite1600_Tax_Rule_Edit_Form extends Mage_Adminhtml_Block_Widget_Form
{
    /**
     * Init class
     *
    ...
    public function __construct()
    {
        ...
        $this->setTitle(Mage::helper('udropship')->__('Tax Rule Information'));
    }
    ...
    protected function _prepareForm()
    {
        ...
        $fieldset   = $form->addFieldset('base_fieldset', array(
            'legend'    => Mage::helper('udropship')->__('Tax Rule Information')
        ));
        ...
        $vendorClasses = Mage::getModel('tax/class')
            ->getCollection()
            ->setClassTypeFilter(Unirgy_DropshipVendorTax_Model_Source::TAX_CLASS_TYPE_VENDOR)
            ->toOptionArray();
        ...
        $fieldset->addField('tax_customer_class', 'multiselect',
            array(
                ...
                'label'     => Mage::helper('udropship')->__('Customer Tax Class'),
                'class'     => 'required-entry',
        ...
        $fieldset->addField('tax_product_class', 'multiselect',
            array(
                ...
                'label'     => Mage::helper('udropship')->__('Product Tax Class'),
                'class'     => 'required-entry',
        ...
        $fieldset->addField('tax_vendor_class', 'multiselect',
            array(
                'name'      => 'tax_vendor_class',
                'label'     => Mage::helper('udropship')->__('Vendor Tax Class'),
                'class'     => 'required-entry',
                'values'    => $vendorClasses,
                'value'     => $model->getVendorTaxClasses(),
                'required'  => true,
            )
        );
        ...
        $fieldset->addField('tax_rate', 'multiselect',
            array(
                'name'      => 'tax_rate',
                'label'     => Mage::helper('udropship')->__('Tax Rate'),
                'class'     => 'required-entry',
                'values'    => $rates,
        ...
        $fieldset->addField('priority', 'text',
            array(
                'name'      => 'priority',
                'label'     => Mage::helper('udropship')->__('Priority'),
                'class'     => 'validate-not-negative-number',
                'value'     => (int) $model->getPriority(),
                'required'  => true,
                'note'      => Mage::helper('udropship')->__('Tax rates at the same priority are added, others are compounded.'),
            )
        );
