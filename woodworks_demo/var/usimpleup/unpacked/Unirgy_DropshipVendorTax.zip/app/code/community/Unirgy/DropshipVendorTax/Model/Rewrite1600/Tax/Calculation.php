 ...
 */
class Unirgy_DropshipVendorTax_Model_Rewrite1600_Tax_Calculation extends Mage_Core_Model_Abstract
{
    ...
    public function getVendorTaxClasses($ruleId)
    {
        if (!isset($this->_vtc[$ruleId])) {
            $this->_vtc[$ruleId] = $this->getResource()->getDistinct('vendor_tax_class_id', $ruleId);
        }
    ...
    {
        if (!$request->getCountryId() || !$request->getCustomerClassId() || !$request->getProductClassId() || !$request->getVendorClassId()) {
            return 0;
        }
    ...
    public function getRatesForAllVendorTaxClasses($request)
    {
        return $this->_getRates($request, 'vendor_class_id', Unirgy_DropshipVendorTax_Model_Source::TAX_CLASS_TYPE_VENDOR);
    }
