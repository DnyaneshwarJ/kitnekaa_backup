...
class Unirgy_DropshipTierShipping_Block_ProductAttribute_Renderer_Rates extends Mage_Adminhtml_Block_Widget implements Varien_Data_Form_Element_Renderer_Interface
{
    ...
    public function __construct()
    {
        ...
        if (!$this->getTemplate()) {
            ...
            if ($curProd && $curProd->getData('_edit_in_vendor')) {
                ...
                $this->setTemplate('udtiership/product/form_field/v2/rates.phtml');
            }
        }
    }
    ...
    {
        return Mage::helper('udtiership')->isUseAdditional($this->getCalculationMethod()) && Mage::helper('udtiership')->isV2Rates() && !Mage::helper('udtiership')->isV2SimpleConditionalRates();
    }

    public function isShowHandlingColumn()
    {
        return Mage::helper('udtiership')->isUseHandling($this->getHandlingApply()) && Mage::helper('udtiership')->isV2Rates() && !Mage::helper('udtiership')->isV2SimpleRates() && !Mage::helper('udtiership')->isV2SimpleConditionalRates();
    }
