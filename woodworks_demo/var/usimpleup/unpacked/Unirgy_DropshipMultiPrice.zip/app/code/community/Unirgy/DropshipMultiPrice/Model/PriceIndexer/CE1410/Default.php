 ...
 */
class Unirgy_DropshipMultiPrice_Model_PriceIndexer_CE1410_Default
    ...
    implements Mage_Catalog_Model_Resource_Eav_Mysql4_Product_Indexer_Price_Interface
{
    ...
    public function getTypeId()
    {
        if (is_null($this->_typeId)) {
            Mage::throwException(Mage::helper('udropship')->__('A product type is not defined for the indexer.'));
        }
        return $this->_typeId;
    }
    ...
    {
        if (method_exists($this, 'useDisableKeys')) $this->useDisableKeys(false);
    ...
    protected function _prepareFinalPriceData($entityIds = null)
    {
        ...
        $_mpHlp = Mage::helper('udmultiprice');
        ...
        $uvSpecialFromDate    = $_mpHlp->getDatePartSql($uvSpecialFrom);
        $uvSpecialToDate      = $_mpHlp->getDatePartSql($uvSpecialTo);

        $uvSpecialFromUse     = $_mpHlp->getCheckSql("{$uvSpecialFromDate} <= {$currentDate}", '1', '0');
        $uvSpecialToUse       = $_mpHlp->getCheckSql("{$uvSpecialToDate} >= {$currentDate}", '1', '0');
        $uvSpecialFromHas     = $_mpHlp->getCheckSql("{$uvSpecialFrom} IS NULL", '1', "{$uvSpecialFromUse}");
        $uvSpecialToHas       = $_mpHlp->getCheckSql("{$uvSpecialTo} IS NULL", '1', "{$uvSpecialToUse}");
        $uvFinalPrice         = $_mpHlp->getCheckSql("{$uvSpecialFromHas} > 0 AND {$uvSpecialToHas} > 0"
            . " AND {$uvSpecialPrice} < {$uvPrice}", $uvSpecialPrice, $uvPrice);

        $csPrice = $_mpHlp->getCheckSql("uvp.vendor_price is null", $finalPrice, $uvFinalPrice);
        foreach ($canStates as $csKey=>$csLbl) {
            ...
            $select->columns(array($csMinKey=>$csMinPrice));
            $select->columns(array($csMaxKey=>$csMaxPrice));
            $select->columns(array($csCntKey=>$csCnt));
        }
        ...
        $select->group(array('entity_id', 'customer_group_id', 'website_id'));
