var UnirgySlider=Class.create({initialize:function(options){this.options={moveDuration:.5,moveStep:1,prevClass:'arrow-left',nextClass:'arrow-right',startPosition:0,cycle:'none',cycleDuration:10}
...
if(this.options.itemsBox&&this.options.itemBoxWidth&&$(this.options.itemsBox)){$(this.options.itemsBox).setStyle({width:(this.itemCnt*this.options.itemBoxWidth)+'px'});}
...
for(i=0;i<Math.ceil(this.itemCnt/this.options.moveStep);i++){if(this.options.navItemHtml){navItemHtml=this.options.navItemHtml;}else if(-1!=['ul','ol'].indexOf(this.itemContainer.tagName.toLowerCase())){navItemHtml='<li><a href="javascript:void(0);">&bull;</a></li>';}else{navItemHtml='<a href="javascript:void(0);">&bull;</a>';}
this.navigationContainer.insert({bottom:navItemHtml})
this.navigationItems.push(this.navigationContainer.childElements().last())}}}
if(this.navigationItems&&this.navigationItems.length>0){for(i=0;i<this.navigationItems.length;i++){this.navigationItems[i].observe('click',this.onMoveByIdx.bindAsEventListener(this,i*this.options.moveStep))}}
...
this.navigationItems[Math.ceil(this.currentIdx/this.options.moveStep)].addClassName('active')}catch(e){}},onMovePrev:function(e){this.stopCycle()
...
if(this.currentIdx+this.options.itemsInContainer<this.itemCnt){this.moveLock=true
this.currentIdx+=this.options.moveStep
