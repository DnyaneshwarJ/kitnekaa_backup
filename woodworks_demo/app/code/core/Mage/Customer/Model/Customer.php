...
class Mage_Customer_Model_Customer extends Mage_Core_Model_Abstract
{
    ...
    public function authenticate($login, $password)
    {
       ...
       /* $activate = $this->getActivateaccount();
		$phoneverify = $this->getPhonenoapproved();
        $passwordstatus =$this->getPasswordstatus();*/
        ...
        }/*else if ($activate != 374) {
            throw Mage::exception('Mage_Core', Mage::helper('customer')->__('This account is not activate.'),
                self::EXCEPTION_EMAIL_NOT_CONFIRMED
            );
        }else if ($passwordstatus != 376) {
            throw Mage::exception('Mage_Core', Mage::helper('customer')->__('This account is not activate.'),
                self::EXCEPTION_EMAIL_NOT_CONFIRMED
            );
        }
        else if ($phoneverify != 372) {
            throw Mage::exception('Mage_Core', Mage::helper('customer')->__('This account is not activate.'),
                self::EXCEPTION_EMAIL_NOT_CONFIRMED
            );
        }*/
