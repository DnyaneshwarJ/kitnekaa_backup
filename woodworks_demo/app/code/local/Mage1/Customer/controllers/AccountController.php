...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginPostAction()
    {
      ...
      /*  echo '<pre>';
        print_r($this->_getHelper('customer')->getEmailConfirmationUrl($login['username']));
        echo '<pre>';
        echo $value;
        echo 'sss';
        exit();*/
        if($redirect == 1){
            $redirection_url = '<?php echo Mage::getBaseUrl(): ?>/index.php/verification/index?q='.$login['username'];
    ...
    protected function _successProcessRegistration(Mage_Customer_Model_Customer $customer)
    {
        ...
        if ($customer->isConfirmationRequired()) {
			...
			if($sessionCustomer->isLoggedIn()) {
				  ...
				  $session->addSuccess($this->__('Successfully added New user in Company'));
				  $url = '<?php echo Mage::getBaseUrl(): ?>/index.php/users?add=1';
				} else {
				   $url = '<?php echo Mage::getBaseUrl(): ?>/index.php/verification?q='.$email;
				}
