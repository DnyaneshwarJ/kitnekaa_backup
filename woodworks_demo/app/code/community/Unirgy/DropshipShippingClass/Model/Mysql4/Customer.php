...
class Unirgy_DropshipShippingClass_Model_Mysql4_Customer extends Mage_Core_Model_Mysql4_Abstract
{
    public function _construct()
    {
        $this->_init('udshipclass/customer', 'class_id');
    }
    ...
    protected function _afterSave(Mage_Core_Model_Abstract $object)
    {
        ...
        $conn->delete(
            $this->getTable('udshipclass/customer_row'),
        ...
        if (($rows = $object->getRows()) && is_array($rows)) {
            ...
            Mage::getResourceSingleton('udropship/helper')->multiInsertOnDuplicate(
                $this->getTable('udshipclass/customer_row'), $rows, array('postcode','region_id')
            );
            }
        }
    }
