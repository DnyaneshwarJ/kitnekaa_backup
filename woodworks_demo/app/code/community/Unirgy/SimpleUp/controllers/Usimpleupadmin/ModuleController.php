...
class Unirgy_SimpleUp_Usimpleupadmin_ModuleController extends Mage_Adminhtml_Controller_Action
{
...
exit;
    ...
    public function postAction()
    {
        ...
        if (($m = Mage::getConfig()->getNode('modules/Unirgy_SimpleLicense')) && $m->is('active')) {
            ...
            case Mage::helper('usimplelic')->__('Add license key'):
                $this->_forward('addLicense', 'usimplelicadmin_license', Mage::app()->getFrontController()->getRouterByRoute('adminhtml')->getFrontNameByRoute('adminhtml'));
                break;
            }
        }
    }
    ...
    {
        try {
            /* @var $installer Unirgy_SimpleUp_Model_Resource_Setup */
            $installer = new Unirgy_SimpleUp_Model_Resource_Setup('usimpleup_setup');
            $installer->reinstall();
