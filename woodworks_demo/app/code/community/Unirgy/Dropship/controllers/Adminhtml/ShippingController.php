...
class Unirgy_Dropship_Adminhtml_ShippingController extends Mage_Adminhtml_Controller_Action
{
    public function indexAction()
    {
        ...
        $this->_addBreadcrumb(Mage::helper('udropship')->__('Shipping Methods'), Mage::helper('adminhtml')->__('Shipping Methods'));
        ...
        if ( $this->getRequest()->getPost() ) {
            try {
                ...
                if ($postedCount>1 && $hasWildcard) {
                    Mage::throwException(
                        $hlp->__('Only one system method could be selected when using "* Any available" wildcard method')
                    );
                }
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Shipping Method was successfully saved'));
    ...
    public function deleteAction()
    {
        if(($id = $this->getRequest()->getParam('id')) > 0 ) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('adminhtml')->__('Shipping Method was successfully deleted'));
                $this->_redirect('*/*/');
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
                $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($certIds)) {
            $this->_getSession()->addError($this->__('Please select shipping method(s)'));
        }
        else {
            try {
