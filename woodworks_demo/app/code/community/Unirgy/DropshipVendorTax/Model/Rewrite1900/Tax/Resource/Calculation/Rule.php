 ...
 */
class Unirgy_DropshipVendorTax_Model_Rewrite1900_Tax_Resource_Calculation_Rule extends Mage_Core_Model_Resource_Db_Abstract
{
    /**
    ...
    protected function _initUniqueFields()
    {
        $this->_uniqueFields = array(array(
            'field' => array('code'),
            'title' => Mage::helper('udropship')->__('Code'),
        ));
        return $this;
    }
    ...
    public function fetchUdRuleCodes($rateId, $customerTaxClassId, $productTaxClassId, $vendorTaxClassId)
    {
        ...
        $select = $adapter->select()
            ...
            ->where('main.customer_tax_class_id in (?)', $customerTaxClassId)
            ->where('main.product_tax_class_id in (?)', $productTaxClassId)
            ->where('main.vendor_tax_class_id in (?)', $vendorTaxClassId)
            ->distinct(true);
