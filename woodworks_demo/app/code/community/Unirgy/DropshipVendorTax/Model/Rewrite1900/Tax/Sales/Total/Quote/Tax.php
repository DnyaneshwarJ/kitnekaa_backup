 ...
 */
class Unirgy_DropshipVendorTax_Model_Rewrite1900_Tax_Sales_Total_Quote_Tax extends Mage_Sales_Model_Quote_Address_Total_Abstract
{
    /**
     * Tax module helper
     *
    ...
    protected function _udCalculateShippingTaxByRate(
        Mage_Sales_Model_Quote_Address $address, $rate, $appliedRates, $taxId, $vId, &$shipTaxTotal)
    {
        $inclTax = $address->getIsShippingInclTax();
        $shipping       = $shipTaxTotal['taxable'];
        $baseShipping   = $shipTaxTotal['baseTaxable'];
        $rateKey = ($taxId == null) ? (string)$rate : $taxId;
        ...
        $shipTaxTotal['baseTax'] = @$shipTaxTotal['baseTax'] + $baseTax;
        $shipTaxTotal['tax'] = @$shipTaxTotal['tax'] + $tax;
        ...
        $shipTaxTotal['hiddenTax'] = @$shipTaxTotal['hiddenTax'] + $hiddenTax;
        $shipTaxTotal['baseHiddenTax'] = @$shipTaxTotal['baseHiddenTax'] + $baseHiddenTax;
    ...
    protected function _calculateShippingTax(Mage_Sales_Model_Quote_Address $address, $taxRateRequest)
    {
        ...
        $discountAmount     = $address->getShippingDiscountAmount();
        $baseDiscountAmount = $address->getBaseShippingDiscountAmount();
        $discountAmount = $discountAmount/max(1,count($shipTaxTotals));
        $baseDiscountAmount = $baseDiscountAmount/max(1,count($shipTaxTotals));

        foreach ($shipTaxTotals as $vId => &$shipTaxTotal) {

        Mage::helper('udtax')->setVendorClassId($taxRateRequest, $vId);
        $appliedRates = $this->_calculator->getAppliedRates($taxRateRequest);

        if ($inclTax) {
            $this->_udCalculateShippingTaxByRate($address, $rate, $appliedRates, null, $vId, $shipTaxTotal);
        } else {
            foreach ($appliedRates as $appliedRate) {
                $taxRate = $appliedRate['percent'];
                $taxId = $appliedRate['id'];
                $this->_udCalculateShippingTaxByRate($address, $taxRate, array($appliedRate), $taxId, $vId, $shipTaxTotal);
            }
        }
        ...
        foreach ($shipTaxTotals as $shipTaxTotal) {
            $tax += @$shipTaxTotal['tax'];
            $baseTax += @$shipTaxTotal['baseTax'];
        }
    ...
    )
    {
        ...
        Mage::helper('udtax')->setVendorClassId($taxRateRequest, $item);
        $rate = $this->_calculator->getRate($taxRateRequest);
    ...
    public function fetch(Mage_Sales_Model_Quote_Address $address)
    {
        ...
        if (($amount != 0) || ($this->_config->displayCartZeroTax($store))) {
            $address->addTotal(array(
                'code' => $this->getCode(),
                'title' => Mage::helper('udropship')->__('Tax'),
        ...
        if ($this->_config->displayCartSubtotalBoth($store) || $this->_config->displayCartSubtotalInclTax($store)) {
            ...
            $address->addTotal(array(
                'code' => 'subtotal',
                'title' => Mage::helper('udropship')->__('Subtotal'),
    ...
    {
        return Mage::helper('udropship')->__('Tax');
    }
}
